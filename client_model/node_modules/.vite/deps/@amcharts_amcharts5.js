...
var NumberFormatter = class extends Entity {
  ...
  applyFormat(value, details) {
    ...
    if (mods.indexOf("b") !== -1) {
      let a3 = this.applyPrefix(value, this.get("bytePrefixes"), mods.indexOf("!") !== -1);
      value = a3[0];
      prefix = a3[1];
      suffix = a3[2];
      ...
      let a3 = this.applyPrefix(value, value < this.get("smallNumberThreshold") ? this.get("smallNumberPrefixes") : this.get("bigNumberPrefixes"), mods.indexOf("!") !== -1);
      value = a3[0];
      prefix = a3[1];
      suffix = a3[2];
      if (details.modSpacing) {
    ...
    let res = "";
    let a2 = numberToString(value).split(".");
    let ints = a2[0];
...
var Matrix = class {
  constructor(a2 = 1, b = 0, c = 0, d = 1, tx = 0, ty = 0) {
...
function splitArcFlags(args) {
  for (let i = 0; i < args.length; i += 7) {
    ...
    let flag = args[index];
    if (flag.length > 1) {
      const a2 = /^([01])([01])(.*)$/.exec(flag);
      if (a2 !== null) {
        args.splice(index, 0, a2[1]);
        ++index;
        args.splice(index, 0, a2[2]);
        ++index;
        if (a2[3].length > 0) {
          args[index] = a2[3];
        } else {
    ...
    if (layerDirty || interactive) {
      ...
      each(this._operations, (op) => {
        ...
        if (interactive) {
          ...
          op.colorize(ghostContext, color2);
        }
      });
    }
  }
...
var CanvasText = class extends CanvasDisplayObject {
  ...
  _addBounds(bounds) {
    if (this.visible && this.isMeasured) {
      const x2 = this._measure(this.getLayer());
...
var CanvasRendererEvent = class {
  constructor(event, originalPoint, point5, bbox) {
...
var CanvasRenderer = class extends ArrayDisposer {
  ...
  */
  ...
  getEvent(originalEvent, adjustPoint = true) {
    ...
    return new CanvasRendererEvent(originalEvent, originalPoint, point5, this._adjustBoundingBox(bbox));
  }
  _getHitTarget(point5, bbox, target) {
    if (bbox.width === 0 || bbox.height === 0 || point5.x < bbox.left || point5.x > bbox.right || point5.y < bbox.top || point5.y > bbox.bottom) {
      return;
    }
    if (!target || !this._layerDom.contains(target)) {
      return;
    }
    const pixel = this._ghostLayer.getImageData(point5, bbox);
...
var GhostLayer = class {
  ...
  getImageData(point5, bbox) {
    return this.context.getImageData(
      // TODO should this round ?
      Math.round((point5.x - bbox.left) / bbox.width * this._width),
      Math.round((point5.y - bbox.top) / bbox.height * this._height),
      1,
      1
    );
  }
...
var Root = class _Root {
  ...
  _showBranding() {
    if (!this._logo) {
      ...
      const a2 = logo.children.push(Graphics.new(this, {
  ...
  _invalidateTabindexes() {
    ...
    this._tabindexes.sort((a2, b) => {
      const aindex = a2.get("tabindexOrder", 0);
    ...
    if (this.isDirty("points")) {
      ...
      if (points) {
        for (let i = 1, len = points.length; i < len; i++) {
          const point5 = points[i];
          minX = Math.min(minX, point5.x);
          maxX = Math.max(maxX, point5.x);
          minY = Math.min(minY, point5.y);
          maxY = Math.max(maxY, point5.y);
        }
      }
  ...
  _draw() {
    ...
    if (points && previousPoints) {
      ...
      for (let i = 1, len = points.length; i < len; i++) {
        ...
        this._display.lineTo(prevPoint.x + (point5.x - prevPoint.x) * morphProgress, prevPoint.y + (point5.y - prevPoint.y) * morphProgress);
      }
...
var LinearGradient = class extends Gradient {
  ...
  getFill(target) {
    ...
    let cos3 = cos(rotation);
    let sin3 = sin(rotation);
    let w = cos3 * (r - l);
    let h = sin3 * (b - t);
    let longer = Math.max(w, h);
    const gradient = this._root._renderer.createLinearGradient(l, t, l + longer * cos3, t + longer * sin3);
...
var Legend = class extends Series {
  ...
  processDataItem(dataItem) {
    ...
    if (itemContainer) {
      ...
      this.children.values.sort((a2, b) => {
        const targetA = a2.dataItem.dataContext;
...
var RadialGradient = class extends Gradient {
  ...
  getFill(target) {
    ...
    const gradient = this._root._renderer.createRadialGradient(x2, y2, 0, x2, y2, radius);
    const stops = this.get("stops");
    if (stops) {
      let i = 0;
      each(stops, (stop) => {
  ...
  _draw() {
    ...
    if (!gap) {
      ...
      for (let i = -count; i < count * 2; i++) {
        const y2 = Math.round(i * step - step / 2) + 0.5;
...
var SpriteResizer = class extends Container {
  ...
  _updatePositions() {
    ...
    if (sprite) {
      ...
      this.setAll({ x: sprite.x() + d * (cxr - 0.5) * cos(a2) - d * (cyr - 0.5) * sin(a2), y: sprite.y() + d * (cyr - 0.5) * cos(a2) + d * (cxr - 0.5) * sin(a2), width: w, height: h, rotation: a2 });
